...
\begin{longtable}{ll p{5cm}} 
    ...
    \textbf{wheel\_status(i)}   & WHEEL\_DEGRADED\_ENCODER  & Quando viene dato un riferimento di velocità ma gli rpm sono nulli per un certo periodo di tempo,
                                                            infatti per impostare questo valore si considera anche l'inerzia delle ruote. Questa condizione è 
                                                            rilevata da $has\_no\_feedback(i)$. \\\\

                                & WHEEL\_CRITICAL\_MOTOR    & Quando alla condizione degradata si aggiunge un'incoerenza del sensore IMU. \\\\
                                & WHEEL\_OK                 & Quando non ci sono ne condizioni critiche ne degradate, imposta questo valore. \\ \hline
    



    \textbf{b2\_sup\_status}    & SUP\_DEGRADED             & Questo valore indica che la Board 2 non sta operando con continuità. Il sistema
                                                              monitora l'effettivo 'battito cardiaco' (heartbeat) del supervisore remoto. Se la media degli ultimi 10 intervalli di comunicazione supera i 40 ms, 
                                                              il sistema rileva un timeout o un'assenza di risposta, segnalando lo stato di degrado del supervisore. \\\\
                                & SUP\_CRITICAL             & Se l'intervallo di tempo dall'ultima comunicazione con Board2 supera i $120\,\mathrm{ms}$, imposta questo valore. \\\\
    ...
    \textbf{rx\_status}         & RX\_DEGRADED              & Questo valore identifica una comunicazione instabile. Sebbene il collegamento fisico sia attivo, fattori quali errori di checksum (CRC) 
                                                                o anomalie nella lunghezza dei pacchetti impediscono l'aggiornamento della variabile data\_last\_valid\_ms. Il sistema monitora la qualità del link calcolando la media mobile 
                                                                degli ultimi 10 intervalli di ricezione valida; se tale media supera la soglia critica di $40$ ms, viene segnalato il degrado della ricezione.\\\\
...
\end{figure}
L'autorizzazione a Board 2 per il controllo del rover è regolato dalla variabile $give\_b2\_actuation$ secondo la seguente logica:
\begin{itemize}
\item \textbf{Stato Critico:} $give\_b2\_actuation = 0$. Il relè è aperto e il movimento viene gestito da Board 2.
\item \textbf{Stato Nominale:} $give\_b2\_actuation = 1$. Il relè è chiuso e il movimento viene gestito da Board 1.
\end{itemize}
Tale meccanismo agisce come un dispositivo di sicurezza hardware (interlock) basato sullo stato del supervisore. come mostrato in Figura~\ref{fig:rele_b2}.
\begin{figure}[H]
