...
\section{Funzionamento generale di ciascuna board}
...
\subsection{Interazione tra le due Board}
...
Board 2 e ricezione della Board 1, evidenziando quali sono i dati che vengono trasmessi e ricevuti tra le due board.
...
\begin{figure}[H]
    ...
    \label{fig:comunicazione_board}
\end{figure}

 Allo stesso modo, nella Figura~\ref{fig:comunicazione_board12} è mostrata l'interazione tra i task di trasmissione della Board 1 e ricezione della Board 2.
...
Il supervisore della Board 1 è implementato anch'esso tramite una macchina a stati sviluppata in Simulink Stateflow. Questo task ha il compito di supervisionare l'integrità dell'hardware e di orchestrare le logiche di movimento avanzate.
...
\begin{itemize}
\item \textbf{Diagnostica e Fault Masking:} Il sistema aggrega i guasti rilevati in maschere di errore (\textit{Critical} e \textit{Degraded}). Queste maschere permettono al supervisore di inibire determinate manovre o di limitare la potenza del sistema in caso di anomalie termiche o di tensione, garantendo la protezione della componentistica meccanica ed elettronica.
\item \textbf{Gestione delle Manovre Complesse:} A differenza della Board 2, focalizzata sull'evitamento ostacoli, la Board 1 gestisce comandi di alto livello come la \textit{Rotazione a 180 gradi} assistita e la procedura di \textit{Emergency Stop}, monitorando l'arresto effettivo delle ruote tramite i sensori di velocità.
\item \textbf{Arbitraggio dell'Attuazione:} Il supervisore valuta costantemente l'affidabilità della comunicazione inter-board. Qualora rilevi un'instabilità critica nel proprio sottosistema di controllo ma una comunicazione valida con l'altra scheda, è in grado di cedere i privilegi di attuazione alla Board 2, realizzando una strategia di ridonanza funzionale che aumenta la resilienza del rover.
\end{itemize}
