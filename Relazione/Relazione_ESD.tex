...
\end{figure}
Il modulo determina la direzione del rover (\textit{B2Decision}) e verifica la coerenza tra la stima odometrica (velocità ruote) e i dati dell'IMU. Inoltre, qualora autorizzato o in caso di mancata comunicazione con la Board 1,
calcola i riferimenti di velocità lineare e angolare.

L'architettura è composta da 5 parti principali:
\begin{itemize}
	\item \textbf{Rilevamento anomalie locali}: monitora lo stato di comunicazione con ESP32, IMU e Board 1, classificandola come \texttt{OK}, \texttt{DEGRADED} o \texttt{CRITICAL}.
	\item \textbf{Generazione maschere di errore}: aggrega le anomalie rilevate nelle maschere locali (\textit{fault masks}), per rappresentare lo stato di salute della Board 2.
	\item \textbf{Logica che abilita l'attuazione}: autorizza la Board 2 all'attuazione in caso di interruzione della comunicazione con la Board 1 o su richiesta esplicita di quest'ultima.
	\item \textbf{Gestione ostacoli}: analizza i dati dei sensori sonar per correggere il comando di movimento in presenza di ostacoli statici o in movimento.
	\item \textbf{Calcolo riferimenti e stato globale}: ricostruisce lo stato del sistema combinando le maschere critiche delle due board. Se abilitata all'attuazione, la Board 2 genera i riferimenti di velocità dimezzandone
	i valori nominali per operare in modalità degradata.
\end{itemize}
...
\begin{itemize}  
	\item \textbf{Board1\_Data}: è l'ultimo snapshot di ricezione ($RxSnapshot$) aggiornato dal task di ricezione di Board2. I segnali utilizzati dal supervisore sono:
	\begin{itemize}
	...
	\begin{figure}[H]
		...
		\label{fig:board_health_struct}
	\end{figure}
	\item \textbf{Sonars}: è l'ultimo snapshot delle distanze dagli ostacoli rilevati. La struttura dati è la seguente:
	\begin{figure}[H]
		\centering
	...
	\end{figure}
	\item \textbf{now\_ms}: rappresenta il tempo corrente in millisecondi, utilizzato per la gestione delle temporizzazioni e il rilevamento di timeout.
	\item \textbf{last\_valid\_b1\_ms}: rappresenta il tempo, in millisecondi, dell'ultimo
	istante in cui la comunicazione con la Board 1 è stata considerata valida. Questo valore viene aggiornato quando 
	vengono ricevuti nuovi dati coerenti dalla Board 2 ed è utilizzato dal supervisore per rilevare
	eventuali fault o relativi alla comunicazione.
\end{itemize}
...
I segnali in output che fornisce sono:
\begin{itemize}
	\item \textbf{B2Decision}: è il comando generato dal supervisore della Board 2 in funzione degli ostacoli rilevati. I valori che può assumere sono:
	\begin{figure}[H] 
		\centering
		\includegraphics[width=0.3\textwidth]{images/SupervisoreB1/CMD\_BOARD2}
		\caption{Comandi in uscita dal supervisore della Board 2.}
	...
	\end{figure}    
	\item \textbf{critical\_mask \& degraded\_mask}: sono due mask di errore che rappresentano le anomalie critiche e degradate rilevate dalla Board 2.
	...
	\item \textbf{autorized\_to\_send\_command}: rappresenta un segnale booleano che indica se la Board2 può inviare comandi di movimento al rele collegato ai driver motori.
	...
	\item \textbf{current\_action}: se Board 2 è abilitata ad attuare, rappresenta un segnale che indica l'azione attualmente intrapresa dal supervisore.
\end{itemize}

La corrispondenza tra le fault mask e le anomalie rilevate è riportata nella Tabella~\ref{tab:fault_mapping_B2}.                                                                   
\begin{table}[H]
	\centering
	\begin{tabular}{lll}
		\hline
...
Analogamente al supervisore di Board 1, la rilevazione dei faults locali è implementata dal Chart \textbf{Monitor Boad Local Status}. 
Questo blocco riceve in ingresso gli snapshot agigornati delle diverse periferiche e produce in uscita lo stato di ciascun 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{images/SupervisoreB2/monitorFualtsB2}
	\caption{\small{Sottoblocchi che si occupano di rilevare anomalie nelle periferiche monitorate da Board 2.}}
	\label{fig:gestione_faults_b2}
\end{figure}

Questi sottoblocchi operano in modo indipendente e in parallelo, permettendo al supervisore di valutare contemporaneamente
lo stato di tutte le periferiche monitorate.

\paragraph{MonitorBLE}
Monitora e rileva un ritardo eccessivo nell'aggiornamento dei dati ricevuti dall'ESP32. L'output associato a questo sottosistema è \textbf{esp\_status} e può assumere i seguenti valori:
\begin{itemize}
	\item \texttt{ESP\_DEGRADED} : se la media mobile degli intervalli di ricezione dati dall'ESP32 supera la soglia di $40\,\mathrm{ms}$. 
	\item \texttt{ESP\_CRITICAL}: se la media mobile degli intervalli di ricezione dati dall'ESP32 supera la soglia di $120\,\mathrm{ms}$. 
	\item \texttt{ESP\_OK}       : quando la frequenza media di aggiornamento rientra nei limiti nominali di sicurezza stabiliti. 
\end{itemize}
	
\paragraph{MonitorBoard1Supervisor}
Verifica che il supervisore della Board 2 sia attivo e aggiornato correttamente. L'output associato
	a questo sottosistema è \textbf{b1\_sup\_status} e può assumere i seguenti valori:
\begin{itemize}
	\item \texttt{SUP\_DEGRADED} : questo valore indica una discontinuità operativa della Board 1. Il supervisore della Board 2 monitora l'heartbeat (una variabile) del supervisore remoto, 
	...
	\item \texttt{SUP\_CRITICAL} : se l'intervallo di tempo dall'ultimo aggiornamento dell'heartbeat del supervisore di Board1 supera i $120\,\mathrm{ms}$, imposta questo valore. 
		
	\item \texttt{SUP\_OK}       : quando non ci sono ne condizioni critiche ne degradate imposta questo valore. 
\end{itemize}
\paragraph{MonitorIMU}
Monitora e rileva un ritardo eccessivo nell'aggiornamento dei dati ricevuti dall'IMU. L'output associato a questo sottosistema è \textbf{imu\_status} e può assumere i seguenti valori:
\begin{itemize}
	\item \texttt{IMU\_DEGRADED} : se la media mobile degli intervalli di aggiornamento dell'angolo di yaw dell'IMU supera i $40\,\mathrm{ms}$. 
	\item \texttt{IMU\_CRITICAL} : se la media mobile degli intervalli di aggiornamento dell'angolo di yaw dell'IMU supera i $120\,\mathrm{ms}$. 
	\item \texttt{IMU\_OK}		 : quando la frequenza media dei dati inerziali rientra nei parametri nominali di funzionamento. 
\end{itemize}  
	
\paragraph{MonitorRx}
Rileva un ritardo eccessivo nell'aggiornamento dei dati ricevuti dalla Board 1. L'output associato a questo sottosistema è \textbf{rx\_status} e può assumere i seguenti valori:
\begin{itemize}
	\item \texttt{RX\_DEGRADED} : questo valore identifica una comunicazione instabile. Sebbene il collegamento fisico sia attivo, fattori quali errori di checksum (CRC) o anomalie nella lunghezza dei pacchetti impediscono l'aggiornamento della variabile data\_last\_valid\_ms. Il sistema monitora la qualità del link calcolando la media mobile degli ultimi 10 intervalli di ricezione valida; se tale media supera la soglia critica di $40$ ms, viene segnalato il degrado della ricezione.
	\item \texttt{RX\_CRITICAL}  : se l'intervallo di tempo dall'ultima ricezione corretta supera i $120\,\mathrm{ms}$, imposta questo valore.
	\item \texttt{RX\_OK}        : quando non ci sono ne condizioni critiche ne degradate imposta questo valore. 
\end{itemize}
...
In particolare, la funzione Matlab \textit{Imu-odometry coherence} monitora tale coerenza prendendo in ingresso i valori del sensore IMU e la velocità delle ruote proveniente 
dallo snapshot di ricezione \textit{RxSnapshot} e dando in uscita la variabile booleana \textit{isMotionConsistent} che può assumere i seguenti valori:
\begin{itemize}
	\item \textbf{0}: indica che c'è una discrepanza significativa tra la rotazione misurata dalla IMU e quella stimata dalla velocità dei motori, suggerendo un possibile guasto o malfunzionamento;
	\item \textbf{1}: indica che i dati della IMU e le stime basate sulla velocità dei motori sono coerenti, suggerendo un funzionamento normale del sistema di movimento.
\end{itemize}
...
\begin{table}[H]
	...
	\begin{tabular}{|c|l|l|l|}
		...
		\hline
		\texttt{0} & ESP32 & \textbf{1} se \textbf{ESP\_CRITICAL} & \textbf{1} se \textbf{ESP\_DEGRADED}\\
		\hline
		\texttt{1} & IMU & \textbf{1} se \textbf{IMU\_CRITICAL} & \textbf{1} se \textbf{IMU\_DEGRADED}\\
		\hline
		\texttt{2} & Ricezione da Board 1 & \textbf{1} se \textbf{RX\_CRITICAL} & \textbf{1} se \textbf{RX\_DEGRADED}\\
		\hline
		\texttt{3} & Supervisore Board 1 & \textbf{1} se \textbf{SUP\_CRITICAL} & \textbf{1} se \textbf{SUP\_DEGRADED}\\
		\hline
		\texttt{4-7} & Reserved & Bit non usati & Bit non usati \\
		\hline
	\end{tabular}
...
\subsection{Logica che consente a Board 2 di attuare}
...
Board 2 può trovarsi nelle condizioni di poter attuare il comando ai motori. Queste condizioni abilitano una variabile booleana
$authorized\_to\_send\_command$ che assume valore positivo o negativo nei seguenti casi:

\begin{itemize}
	\item \textbf{authorized\_to\_send\_command = 1}: Board 2 è autorizzata a inviare comandi di movimento al rover. Questa condizione si attiva quando si verifica almeno uno dei seguenti casi:
	\begin{itemize}
		\item \textbf{Anomalie critiche in ricezione} (\textit{rx\_status = RX\_CRITICAL}): la Board 2 rileva autonomamente un'interruzione del flusso dati.
		\item \textbf{Anomalie critiche del supervisore Board 1} (\textit{b1\_sup\_status = SUP\_CRITICAL}): il monitoraggio remoto della Board 1 fallisce.
		\item \textbf{Richiesta esplicita di switch} (\textit{b1\_requests\_switch = true}): la Board 1 stessa segnala di non poter proseguire. In base alla funzione 
		\texttt{classifyB1CriticalMask}, questa richiesta diventa vera se la Board 1 
		riscontra un errore critico di comunicazione (\texttt{RX\_BIT}) o un timout del 
		supervisore di Board 2(\texttt{B2SUP\_BIT}).
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	\item \textbf{Attuazione concreta}: Board 2 invia i segnali di attuazione e comanda effettivmente i motori quando la Board 1 riscontra un errore critico di comunicazione (\texttt{RX\_BIT}) o 
	un timout del supervisore di Board 2(\texttt{B2SUP\_BIT}). In questo caso Board 1 apre il rele per permettere al segnale di attuazione
	di Board 2 di arrivare ai motori. La Figura~\ref{fig:board1_non_riceve} mostra il caso in cui Board 1 apre il rele a causa di una mancata ricezione da Board 2
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\textwidth]{images/funzionamento_generale/specifiche/Board1_non_riceve}
		\caption{Board 1 non riceve dati dalla Board 2.}
		\label{fig:board1_non_riceve}
	\end{figure}
	
	\item \textbf{Attuazione teorica}: Board 2 invia i segnali di attuazione ma non comanda effettivamente i motori quando la Board 2 riscontra un'interruzione nel flusso di dati da Board 1, quindi quando 
	riscontra un errore in ricezione (\textit{rx\_status = RX\_CRITICAL}). In questo caso Board 2 non può sapere se Board 1 sia ancora in funzione e quindi Board 2 prova 
	comunque ad attuare. Se la comunicazione con Board 1 è ancora stabile, i messaggi che Board 1 riceve da Board 2 le permettono di 
	continuare ad attuare. La Figura~\ref{fig:board2_non_riceve} mostra il caso in cui Board 1 non apre il rele nonostante Board 2 non riesca a comunicare con Board 1.
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\textwidth]{images/funzionamento_generale/specifiche/Board2_non_riceve}
...
\begin{enumerate}
	\item \textit{Board 1 comanda i motori}
	\begin{itemize}
	...
	\end{itemize}
	\item \textit{Board 2 comanda i motori (modalità degradata con Board isolata)}
	\begin{itemize}
...
\subsubsection{Gestione ostacoli con Board 1 attuatrice}
...
Il chart per la gestione degli ostacoli in con Board 1 attuatrice è mostrato in Figura~\ref{fig:chart_B1_attua}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{images/SupervisoreB2/GestioneRilevamentoOstacoli/GestioneOstacoli_StatoNonDegradato}
	\caption{Chart di gestione ostacoli con Board 1 attuatrice.}
	\label{fig:chart_B1_attua}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.49\textwidth}
		\includegraphics[width=\textwidth]{images/SupervisoreB2/GestioneRilevamentoOstacoli/SonarState2}
		\caption{Stato parallelo Sonars}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.49\textwidth}
		
		\includegraphics[width=\textwidth]{"images/SupervisoreB2/GestioneRilevamentoOstacoli/B2 decision state 2"}
...
\begin{enumerate}
	...
	ricevuti, è capace di settare la variabile di output del chart che indica la decisione presa dal supervisore.
	Quindi nello stato \textit{B2Decisions} si determina l'output del chart, che è un numero che varia da $0$ a $4$.
	Gli output determinano le azioni possibili che includono l'arresto immediato del rover o la deviazione del percorso in base al movimento intorno al rover dell'ostacolo. In quest'ultimo caso, la deviazione dura fintanto che il rover non ruota
	di 45° rispetto alla direzione iniziale, verso quella del sonar che per primo ha rilevato l'ostacolo.
...
\subsubsection{Gestione ostacoli con Board 2 attuatrice}

Questo tipo di gestione si verifica quando il controllo passa alla Board 2 (Slave). Ciò avviene quando la Board 1 rileva un errore critico di comunicazione (\texttt{RX\_BIT}) oppure un timeout del supervisore di Board 2.


Il chart per la gestione degli ostacoli quando è Board 2 ad attuare è mostrato in Figura~\ref{fig:chart_B2_attua}.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{images/SupervisoreB2/GestioneRilevamentoOstacoli/GestioneOstacoli_StatoDegradato}
	\caption{Chart di gestione ostacoli quando B2 attua}
	\label{fig:chart_B2_attua}
\end{figure}
...
Il sottosistema \textit{Actuation decision and motion state} viene attivato quando la Board 2 assume la responsabilità dell'attuazione. 
Questo subentro avviene in caso di interruzione della ricezione dei dati di Board 1 o su richiesta esplicita di quest'ultima 
(ad esempio, qualora la Board 1 rilevi un guasto critico nella ricezione da Board 2).
...
In questa modalità, il supervisore gestisce il movimento in regime degradato, limitando la velocità al 50\% del valore nominale (pari a 80 RPM rispetto ai 160 RPM massimi). 
Il sistema impone inoltre un arresto di emergenza immediato al verificarsi di una delle seguenti condizioni:
\begin{itemize}
	\item rilevazione di un ostacolo a una distanza inferiore ai 3 metri;
	\item presenza di un guasto critico nella maschera locale di Board 2;
	\item ricezione di almeno un guasto critico nella critical mask proveniente dalla Board 1 (se il link di comunicazione è ancora attivo).
\end{itemize}

Oltre alla marcia normale condizionata dalle condizioni descritte sopra, il sottosistema ha anche la possibilità di gestire la rotazione
a 180° o la semplice retromarcia, analogamente al sottosistema \textit{Backward\_mode\_toggle} della Board 1.
